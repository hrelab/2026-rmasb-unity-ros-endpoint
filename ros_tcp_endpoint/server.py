...
#  Modified by Jono Jenkens, 2026-02-06: Added syscommand unregister support (remove_subscriber/remove_publisher/remove_ros_service/remove_unity_service), safer syscommand dispatch, and fixed repeat registration errors.
#
...
class TcpServer(Node):
    ...
    def handle_syscommand(self, topic, data):
        ...
        if function is None:
            error_msg = "Don't understand SysCommand '{}'".format(topic)
            self.send_unity_error(error_msg)
        ...
        try:
            ...
            print("JSON decode error in syscommand:", message_json)
            raise e
        function(**params)
...
class SysCommands:
    ...
    def subscribe(self, topic, message_name):
        ...
        if old_node is not None:
            # If Unity re-registers the same subscriber (common across scene loads), treat as idempotent.
            if getattr(old_node, "msg", None) == message_class:
                self.tcp_server.loginfo(
                    "RegisterSubscriber({}, {}) already registered".format(topic, message_class)
                )
                return
    ...
    def publish(self, topic, message_name, queue_size=10, latch=False):
        ...
        if old_node is not None:
            # If Unity re-registers the same publisher (common if multiple scenes contain the same publisher),
            # avoid destroying/recreating the ROS2 node, which triggers rclpy "publisher already registered" warnings.
            if getattr(old_node, "msg", None) == message_class:
                self.tcp_server.loginfo(
                    "RegisterPublisher({}, {}) already registered".format(topic, message_class)
                )
                return
            self.tcp_server.unregister_node(old_node)
    ...
    ################# This is all new code to make unsubscribing and unpublishing work ##############################
    def remove_subscriber(self, topic):
        """Unregister an existing subscriber.

        Unity ROS-TCP-Connector sends the syscommand "__remove_subscriber" with JSON like:
            {"topic": "/some_topic"}
        """
        if topic == "":
            self.tcp_server.send_unity_error(
                "RemoveSubscriber - Can't remove a blank topic name!"
            )
            return

        old_node = self.tcp_server.subscribers_table.pop(topic, None)
        if old_node is None:
            # Idempotent behavior: Unity may remove twice during teardown.
            self.tcp_server.logwarn("RemoveSubscriber({}, ...) - not found".format(topic))
            return
    ...
    def remove_publisher(self, topic):
        if topic == "":
            self.tcp_server.send_unity_error(
                "RemovePublisher - Can't remove a blank topic name!"
            )
            return

        old_node = self.tcp_server.publishers_table.pop(topic, None)
        if old_node is None:
            self.tcp_server.logwarn("RemovePublisher({}, ...) - not found".format(topic))
            return
    ...
    def remove_ros_service(self, topic):
        if topic == "":
            self.tcp_server.send_unity_error(
                "RemoveRosService - Can't remove a blank topic name!"
            )
            return

        old_node = self.tcp_server.ros_services_table.pop(topic, None)
        if old_node is None:
            self.tcp_server.logwarn("RemoveRosService({}, ...) - not found".format(topic))
            return

        self.tcp_server.unregister_node(old_node)
        self.tcp_server.loginfo("RemoveRosService({}) OK".format(topic))

    def remove_unity_service(self, topic):
        if topic == "":
            self.tcp_server.send_unity_error(
                "RemoveUnityService - Can't remove a blank topic name!"
            )
            return

        old_node = self.tcp_server.unity_services_table.pop(topic, None)
        if old_node is None:
            self.tcp_server.logwarn("RemoveUnityService({}, ...) - not found".format(topic))
            return

        self.tcp_server.unregister_node(old_node)
        self.tcp_server.loginfo("RemoveUnityService({}) OK".format(topic))
    #########################################################################################################################
